...
from app.models.payroll_models import PAYROLL_BATCHES_COLLECTION, SALARY_TRANSACTIONS_COLLECTION

def test_100_employee_benchmark_batch():
    # Setup isolated in-memory MongoDB mock database
    db = mongomock.MongoClient()["payroll_sentinel_test"]
    ...
    # Process payroll data through 5-layer engine with MongoDB store
    ...
    # 2. Verify MongoDB Collection Storage
    batch_doc = db[PAYROLL_BATCHES_COLLECTION].find_one({"id": batch.id})
    assert batch_doc is not None, "Batch document not saved in MongoDB"
    assert batch_doc["total_employees"] == 100
    
    tx_count = db[SALARY_TRANSACTIONS_COLLECTION].count_documents({"batch_id": batch.id})
    assert tx_count == 100, f"Expected 100 transactions in MongoDB, got {tx_count}"

    # 3. Verify NO Fabricated Entities (Manager, Device, IPAddress)
    ...
    # 4. Verify Exact Dynamic Node Counts
    ...
    # 5. Verify Exact Dynamic Edge Counts
    ...
    # 6. Verify AC9001 Cluster (5 Employees)
    ...
    # 7. Verify AC9100 Cluster (3 Employees)
    ...
    # 8. Verify Risk Scoring & Deterministic Breakdown for Clean vs Fraud Employees
    ...
    # 9. Verify Financial Amounts Breakdown
    ...
    print("\n✅ MONGODB 100-EMPLOYEE BENCHMARK PIPELINE VERIFICATION PASSED PERFECTLY!")
