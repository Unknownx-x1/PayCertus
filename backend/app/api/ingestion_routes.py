...
def upload_payroll_csv(file: UploadFile = File(...), db = Depends(get_db)):
    ...
    try:
        ...
        batch, graph_payload, narrative, warnings = IngestionService.process_payroll_data(records, file.filename, db)
...
def load_sample_clean(db = Depends(get_db)):
    """Seed & process clean demo payroll run."""
    records = MockDataGenerator.get_clean_payroll()
    batch, graph_payload, narrative, warnings = IngestionService.process_payroll_data(records, "Aug 2026 Regular Payroll (Clean)", db)
    return {
...
def load_sample_fraud(db = Depends(get_db)):
    """Seed & process high-risk fraud ring demo payroll run."""
    records = MockDataGenerator.get_fraud_ring_payroll()
    batch, graph_payload, narrative, warnings = IngestionService.process_payroll_data(records, "Aug 2026 Executive Payroll (Fraud Ring Alert)", db)
    return {
